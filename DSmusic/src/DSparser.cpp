		...
		while (i < getRowCount()) {
			...
			for (size_t j = i; j < getRowCount(); ++j) {
				...
				if (j == i) {
					...
					merged_noteSeq.insert(merged_noteSeq.end(), getNoteSeq(j).begin(), getNoteSeq(j).end());
					merged_noteSlur.insert(merged_noteSlur.end(), getNoteSlur(j).begin(), getNoteSlur(j).end());
